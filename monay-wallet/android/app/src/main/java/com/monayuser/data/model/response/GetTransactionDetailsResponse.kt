...
class GetTransactionDetailsResponse : BaseResponse<GetTransactionDetailsResponse, String, Any>() {
    ...
    override fun doNetworkRequest(
        ...
        networkResponseCallback: NetworkResponseCallback<GetTransactionDetailsResponse>
    ): Disposable {
        val api = ApiFactory.clientWithHeader.create(ApiInterface::class.java)
        return api.getTransactionDetailsAPI(
                "${AppConstants.BEARER} ${appPreference.getSavedUser(
                    appPreference
                ).token}", requestParam["id"].toString()
            )
