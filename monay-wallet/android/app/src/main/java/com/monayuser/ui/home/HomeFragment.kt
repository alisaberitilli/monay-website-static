...
class HomeFragment : BaseFragment<FragmentHomeBinding, HomeViewModel>(), HomeNavigator {
    ...
    fun showDeclineDialog(position: Int) {
        ...
        bottomSheet.findViewById<android.widget.ImageView>(R.id.ic_cross).setOnClickListener { dialog.dismiss() }

        bottomSheet.findViewById<android.widget.Button>(R.id.btn_no).setOnClickListener { dialog.dismiss() }

        bottomSheet.findViewById<android.widget.Button>(R.id.btn_yes).setOnClickListener {
            var reason = bottomSheet.findViewById<android.widget.EditText>(R.id.et_reason).text.toString().trim()
