...
class PaymentMyRequestVM {
    
    // MARK: - Instance properties
    
    var myRequests: [PaymentRequest] = []
    ...
    func myRequestAPI(_ handler: @escaping ((PlaceholderState) -> Void), completion: @escaping (() -> Void)) {
        ...
        APIComponent
            .Profile
            .getMyRequest(parameters: parameters, result: { (result) in
                ...
                case .success(let response):
                    if let data = response.serverResponse as? HTTPParameters,
                       let rows = data[APIKey.rows] as? [HTTPParameters] {
                        ...
                        let myRequests = rows.compactMap { PaymentRequest(JSON: $0) }
                        self.myRequests.append(contentsOf: myRequests)

                        if self.myRequests.isEmpty {
