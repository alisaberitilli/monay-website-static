...
class PendingRequestVM {
    ...
    var pendingRequests: [SupportRequest] = []
                       ...
                       let rows = data[APIKey.rows] as? [HTTPParameters] {
                        ...
                        if rows.isEmpty {
                          ...
                          self.total = data[APIKey.total] as? Int ?? 0
                            let pendingRequest = rows.compactMap { SupportRequest(JSON: $0) }
                            self.pendingRequests.append(contentsOf: pendingRequest)
                            completion(true, "")
                        }
