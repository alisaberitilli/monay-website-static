...
class CountryPicker {
    ...
    static var countryNamesByCode: [Country] = {
        
        var countries = [Country]()
        
        let frameworkBundle = Bundle(for: type(of: CountryPicker()))
        
        guard let jsonPath = frameworkBundle.path(forResource: countryCodeJsonFilePath, ofType: kJson), let jsonData = try? Data(contentsOf: URL(fileURLWithPath: jsonPath)) else { // swiftlint:disable:this line_length
            return countries
        }
        ...
        do {
            if let jsonObjects = try JSONSerialization.jsonObject(with: jsonData, options: JSONSerialization.ReadingOptions.allowFragments) as? NSArray {
                ...
                for jsonObject in jsonObjects {
                    ...
                    guard let code = countryObj[kCode], let phoneCode = countryObj[kDialCode], let name = countryObj[kName] else {
                        return countries
                    }
                    
                    let flagName = "\(flagImagePath)\(code.uppercased())"
