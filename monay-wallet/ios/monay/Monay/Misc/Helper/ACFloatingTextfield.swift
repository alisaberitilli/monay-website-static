...
//  ACFloatingTextField
//
//  Created by Akash Singh Sisodia on 04/01/19.
//  Copyright © 2019 Akash Singh Sisodia. All rights reserved.
//
...
@objc open class ACFloatingTextfield: UITextField {
    ...
    fileprivate var placeholderLabelWidth : NSLayoutConstraint?
    ...
    /// Change FloatexTextview Background color.
    @IBInspectable open var floatedPlaceholderBackgroundColor : UIColor = UIColor.white{
        didSet{
            self.floatTheLabel()
        }
    }
    ...
    //        return CGRect(x:100, y:4, width:bounds.size.width, height:bounds.size.height);
    //    }
    //
...
fileprivate extension ACFloatingTextfield {
    ...
    func addFloatingLabel(){
        ...
        labelPlaceholder?.font = UIFont.init(name: (self.font?.fontName ?? "helvetica")!, size: 14)
        labelPlaceholder?.isHidden = true
        labelPlaceholder?.sizeToFit()
        labelPlaceholder?.backgroundColor = floatedPlaceholderBackgroundColor
...
//        self.setValue(placeHolderColor, forKeyPath: "_placeholderLabel.textColor")
        ...
        let leadingConstraint = NSLayoutConstraint.init(item: labelPlaceholder!, attribute: .leading, relatedBy: .equal, toItem: self, attribute: .leading, multiplier: 1, constant: 10)
        ...
        let topConstraint = NSLayoutConstraint.init(item: labelPlaceholder!, attribute: .top, relatedBy: .equal, toItem: self, attribute: .top, multiplier: 1, constant: -7)
        ...
        placeholderLabelWidth = NSLayoutConstraint.init(item: labelPlaceholder!, attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant:  (labelPlaceholder?.frame.width)!)
        
        self.addConstraints([leadingConstraint,topConstraint])
        labelPlaceholder?.addConstraint(placeholderLabelHeight!)
        labelPlaceholder?.addConstraint(placeholderLabelWidth!)
    ...
    func addErrorPlaceholderLabel() -> Void {
        ...
        labelErrorPlaceholder?.font = UIFont(name: (self.font?.fontName ?? "helvetica")!, size: 14)
...
extension ACFloatingTextfield {
    ...
    private func newBounds(_ bounds: CGRect) -> CGRect {
        ...
        newBounds.origin.x += UIEdgeInsets(top: 0, left: 15, bottom: 0, right: 0).left
        newBounds.origin.y += UIEdgeInsets(top: 0, left: 20, bottom: 0, right: 0).top
        newBounds.size.height -= UIEdgeInsets(top: 0, left: 20, bottom: 0, right: 0).top + UIEdgeInsets(top: 0, left: 20, bottom: 0, right: 0).bottom
        newBounds.size.width -= UIEdgeInsets(top: 0, left: 20, bottom: 0, right: 0).left + UIEdgeInsets(top: 0, left: 20, bottom: 0, right: 0).right
        return newBounds
    }
}
