...
const Otp: React.FC<NoauthStageProps & { email: string; confirmOtp: () => Promise<void> }> = ({ email, confirmOtp }) => {
  ...
  const submitOtp = async () => {
    ...
    const { error } = await minTime(confirmOtp(otp.join("")), 2000);
    
    if (!error) return redirect("/application");
  ...
  return (
    <div className="mb-4 mt-8">
      ...
      <p className="mb-4 text-center leading-none">
        You should receive an email at {email} with a one-time
