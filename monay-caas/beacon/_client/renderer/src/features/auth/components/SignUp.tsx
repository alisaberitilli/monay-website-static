...
import useNoauth, { NoauthStageProps } from "../hooks/useNoauth";
...
const SignUp: React.FC<NoauthStageProps> = ({ goToOtp }) => {
  ...
  const onSubmit: SubmitHandler<SignUpSchema> = async ({
    ...
    const { error } = await root.auth.signUp(email, name, orgName);
