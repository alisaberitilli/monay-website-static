...
export default function MPINSetupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/setup-mpin', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        ...
        toast.error(result.error || 'Failed to setup MPIN');
      }
    } catch (error: any) {
      toast.error('MPIN setup failed. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center">
            {step === 'create'
              ? 'Set PIN - Set up a 6-digit MPIN for secure access'
