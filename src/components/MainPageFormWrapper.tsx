...
interface MainPageFormWrapperProps {
  children: ReactNode;
  formData: any;
  formType: string;
...
export default function MainPageFormWrapper({
  children,
  formData,
  formType,
  onSubmit,
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Call the original submit handler (sends email)
      const emailSuccess = await onSubmit(formDataWithToken);
      
      if (!emailSuccess) {
        throw new Error('Failed to send email');
      }
      
      // Send to Vtiger CRM (don't fail if this doesn't work)
      try {
      ...
      // Reset form after 5 seconds if reset function provided
      if (resetFormData) {
        setTimeout(() => {
          resetFormData();
  ...
  }, [formData, formType, executeRecaptcha, isSubmitting, hasSubmitted, onSubmit, onSuccess, onError, resetFormData, setIsSubmitting]);
  ...
  return (
    <form onSubmit={handleSubmit} className={className}>
      ...
      <button
        ...
        className={submitButtonClassName || `w-full py-4 px-6 font-semibold rounded-lg transition-all duration-200 transform ${
