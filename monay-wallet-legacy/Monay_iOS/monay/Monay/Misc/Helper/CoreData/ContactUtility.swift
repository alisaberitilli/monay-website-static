...
//        }
    ...
    func getFinalNumber(_ phoneNumber: CNLabeledValue<CNPhoneNumber>, compilationClosure: @escaping (_ finalNumber: String, _ countryCode: String) ->()) {
        ...
        do {
            let phone = try phoneUtility.parse(phoneDigits, defaultRegion: countryRegionCode)
            countryCode = phone.countryCode.stringValue
            nationalPhoneNumber = phone.nationalNumber.stringValue
        } catch {
            print(error.localizedDescription)
        }
