...
class FAQVM {
    ...
    func faqAPI(_ handler: @escaping ((PlaceholderState) -> Void), completion: @escaping (() -> Void)) {
        ...
        APIComponent
            .More
            .faqs(parameters: parameters, result: { (result) in
                ...
                case .success(let response):
                    ...
                    if let data = response.serverResponse as? HTTPParameters,
                       let rows = data[APIKey.rows] as? [HTTPParameters] {
                        ...
                        let faqs = rows.compactMap { FAQ(JSON: $0) }
                        self.faqs.append(contentsOf: faqs)

                        if self.faqs.isEmpty {
                            handler(.noResult(message: PlaceholderStateData.noFAQ.title))
                        } else {
                            completion()
                        }
