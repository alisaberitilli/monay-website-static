...
class PaymentDeclineVM {
    
    // MARK: - Instance properties
    
    var declinedRequests: [PaymentRequest] = []
    ...
    func declinedPaymentRequestAPI(_ handler: @escaping ((PlaceholderState) -> Void), completion: @escaping (() -> Void)) {
        ...
        APIComponent
            .Profile
          .paymentRequestType(type: LocalizedKey.declined.value, parameters: parameters, result: { (result) in
                ...
                case .success(let response):
                    if let data = response.serverResponse as? HTTPParameters,
                       let rows = data[APIKey.rows] as? [HTTPParameters] {
                        ...
                        let declinedRequests = rows.compactMap { PaymentRequest(JSON: $0) }
                        self.declinedRequests.append(contentsOf: declinedRequests)
