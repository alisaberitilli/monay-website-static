  ...
  */
  ...
  async signup(req, res, next) {
    try {
      ...
      if (result) {
        req.body.username = result.phoneNumber;
        req.body.phoneCountryCode = result.phoneNumberCountryCode;
   ...
   */
   async sendOtpPrimaryUser(req, res, next) {
    try {
      ...
      console.log('verificationOtp', verificationOtp, phoneNumber);
      let encrypedVerificationOtp = await encryptAPIs.encrypt(`${verificationOtp}`);
      const user = await userRepository.findOne({ phoneNumberCountryCode, phoneNumber, userType: 'user' });
