 ...
 */
  async checkUserExistsOnly(req, res, next) {
    try {
      if (req.params.userId) {
        let { userId } = req.params
        const result = await userRepository.findOne({ id: userId, userType: { [Op.in]: ['merchant', 'user', 'secondaryUser'] } });
        if (result) {
          if (result.status == 'active') {
            req.userInfo = result;
            next();
  ...
  async checkSecondaryUserExists(req, res, next) {
    try {
      ...
      if (type) {
        delete where.id;
        where.userId = id;
        where['$User.user_type$'] = 'secondaryUser';
      }
      if (parentUser) {
        delete where.userId;
        delete where.id;
