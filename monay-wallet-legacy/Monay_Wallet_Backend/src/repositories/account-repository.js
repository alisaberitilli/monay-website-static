        ...
        if (userInfo && !userInfo.qrCode) {
          ...
          if (uniqueCode) {
            ...
            QRCode.toFile(`${fileDir}/qrcode_${uniqueCode}.png`, JSON.stringify({ userId: uniqueCode }), {
              ...
              if (err) throw err
              await userInfo.update({ uniqueCode: uniqueCode, qrCode: `public/uploads/qrcode/qrcode_${uniqueCode}.png` });
            });
          }
        }
      }
  ...
  async checkUserAccountLogin(req) {
    try {
      ...
      if (isEmail) {
        ...
        where.phoneNumberCountryCode = phoneCountryCode;
 ...
 */
  ...
  async adminForgotPassword(req) {
    try {
      ...
      if (user) {
        if (user && (user.userType == 'admin' || user.userType == 'subadmin') && user.status == 'active') {
          req.forgotUser = user;
  ...
  async resetPassword(req) {
    try {
      let { phoneNumberCountryCode, username, otp, newPassword } = req.body;
  ...
  */
  async verifyOtp(req) {
    try {
      let { phoneNumberCountryCode, username, otp } = req.body;
      otp = await encryptAPIs.encrypt(`${otp}`);
      const isEmail = await utility.validateEmail(username);
      let where = {};
      if (isEmail) {
