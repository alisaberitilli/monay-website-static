...
class EmailVerifyResponse : BaseResponse<EmailVerifyResponse, String, Any>() {
    override fun doNetworkRequest(
        requestParam: HashMap<String, Any>,
        appPreference: AppPreference,
        networkResponseCallback: NetworkResponseCallback<EmailVerifyResponse>
    ): Disposable {
        val api = ApiFactory.clientWithHeader.create(ApiInterface::class.java)
        return api.emailVerifyApi("${AppConstants.BEARER} ${appPreference.getSavedUser(appPreference).token}")
