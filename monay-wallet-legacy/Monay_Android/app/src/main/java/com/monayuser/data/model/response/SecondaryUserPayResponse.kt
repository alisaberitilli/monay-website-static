...
import com.monayuser.data.model.bean.SecondaryUserPayBean
...
class SecondaryUserPayResponse : BaseResponse<SecondaryUserPayResponse, String, Any>() {

    var data: SecondaryUserPayBean? = null
    override fun doNetworkRequest(
        requestParam: HashMap<String, Any>,
        appPreference: AppPreference,
        networkResponseCallback: NetworkResponseCallback<SecondaryUserPayResponse>
    ): Disposable {
        ...
        if (requestid == 0) {
            ...
            return api.secondaryUserPayMoney(
...
//            requestParam.remove("saveCard")
            return api.paymentRequestPaySecondaryUser(
                "${AppConstants.BEARER} ${appPreference.getSavedUser(
                    appPreference
                ).token}", requestParam
            )
