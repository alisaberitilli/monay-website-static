...
class SecondaryAccountViewModel : BaseViewModel<SecondaryAccountNavigator>() {
     ...
     */
    internal fun callSecondaryAccountListAPI(showProgress:Boolean,appPreferences: AppPreference,offset:String,limit:String) {
        if (showProgress) {
        ...
        disposable.add(
            SecondaryUserListResponse().doNetworkRequest(prepareRequestHashMapForGetCard(offset,limit),
                appPreferences,
                object : NetworkResponseCallback<SecondaryUserListResponse> {
                    override fun onResponse(getListResponse: SecondaryUserListResponse) {
                        navigator!!.hideProgressBar()
                        navigator!!.showHideLoader()
                        if (getListResponse.isSuccess) {
                            navigator!!.getSecondaryAccountListResponse(getListResponse)
                        } else {
