...
import com.monayuser.data.model.bean.RecentUserListResponseBean
...
class RecentUserListResponse : BaseResponse<RecentUserListResponse, String, Any>() {

    var data: RecentUserListResponseBean? = null
    override fun doNetworkRequest(
        requestParam: HashMap<String, Any>,
        appPreference: AppPreference,
        networkResponseCallback: NetworkResponseCallback<RecentUserListResponse>
    ): Disposable {
        val api = ApiFactory.clientWithHeader.create(ApiInterface::class.java)
        return api.callRecentUserListAPI("${AppConstants.BEARER} ${appPreference.getSavedUser(appPreference).token}", requestParam)
