...
class PaymentRequest extends PureComponent {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        ...
        {
          dataField: 'updatedAt',
          text: props.t('common.paidDeclDateTime'),
          csvText: props.t('common.paidDeclDateTime'),
          csvType: Date,
          headerAlign: 'left',
          align: 'left',
          sort: true,
          formatter: (cell, row) => (row.status !== 'pending') ? dateFormatter(cell)
            : '-',
          csvFormatter: (cell, row) => (row.status !== 'pending') ? csvDateFormatter(cell)
            : '-',
        }
      ]
    }
  ...
  fetchTransaction = async (
    ...
    try {
      const payload = {
        ...ApiEndPoints.getPaymentRequest,
        queryParams
      }
  ...
  render() {
    ...
    const noDataMessage = filterCount > 0 ? textMessages.noDataFound : textMessages.noDataPaymentRequestList

    return (
      <>
        <MetaTags
          title={`${config.NAME_TITLE} | ${t('transaction.paymentRequest')}`}
          description={`${t('transaction.paymentRequest')} of ${config.NAME_TITLE}`}
        />
        <main className='main-content admin-dashboard'>
          <div className='container-fluid'>
              ...
              <div className='left-side'>
                <BreadCrumb
                  bredcrumbs={[
                    ...
                    {
                      name: t('transaction.paymentRequest')
                    }
                  ]}
                />
                <h2 className='page-title text-capitalize mb-0'>
                  {t('transaction.paymentRequest')}
                </h2>
              </div>
                    ...
                    <div className='form-group'>
                      <Form.Item name='status'>
                        <Select className='form-control' placeholder={t('common.status')}>
                          ...
                          <Select.Option value='declined'>{t('common.decline')}</Select.Option>
                        </Select>
                      </Form.Item>
                    </div>
                    <div className='form-group form-group-date'>
                      <Form.Item name='createdAt'>
                        <DatePicker.RangePicker
                          placeholder={[t('common.transactionStartDate'), t('common.transactionEndDate')]}
            ...
            {
              checkUserPermission(this.props.userData, 'payment_request') ?
                <RemoteDataTable
                  data={data}
                  page={page}
                  ...
                  fileName={`Monay_Payment_Request_Listing_${currentDate()}.csv`}
                /> :
                <RemoteDataTable
                  data={data}
                  page={page}
...
export default withTranslation()(connect(mapStateToProps)(PaymentRequest))
