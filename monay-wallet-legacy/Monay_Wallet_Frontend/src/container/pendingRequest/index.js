...
class PendingRequest extends PureComponent {
  ...
  actionFormatter = (cell, row) => {
    ...
    return (
      <a
        className='btn btn-primary ripple-effect  text-uppercase'
        href='/'
        onClick={(e) => {
          e.preventDefault()
          this.setState({
    ...
    this.setState(
      ...
      () => {
        ...
        if (!this.state.isFirstTimeFetching) {
          addPageSizeInURL(page, sizePerPage, this.props.history, { tab: 'pending' })
        }
  ...
  render() {
    ...
    return (
      <>
        <MetaTags
          title={`${config.NAME_TITLE} | ${t('support.pendingReq')}`}
          description={`${t('support.pendingReq')} of ${config.NAME_TITLE}`}
        />
              ...
              <div className='left-side'>
                <BreadCrumb
                  bredcrumbs={[
                    ...
                    {
                      name: t('support.pendingReq')
                    }
                  ]}
                />
                <h2 className='page-title text-capitalize mb-0'>
                  {t('support.pendingReq')}
                </h2>
              </div>
