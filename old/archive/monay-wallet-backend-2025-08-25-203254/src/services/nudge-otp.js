...
class NudgeOTPService {
  constructor() {
    ...
    this.nudgeApiKey = process.env.NUDGE_API_KEY || 'S`dRXF4oct/d2bYBQo23HwgRfp7VKj0XzYNiG7Tu5bMF9typAFylE939/bxaxn8QmnDcqQ19hAjV9mEaRYgmJpAg==';
    this.nudgeSmsApiKey = process.env.NUDGE_SMS_API_KEY || 'S`xLGEiqERfN9IIb/eR52M2MAT5qjAM5Anj011IzsWZmAnzAgYOy6el7BaDTScJNGmTPIOzSNhnDs9+xVoiMHclw==';
  ...
  async sendSMSOTP(phoneNumber, userId) {
    try {
      ...
      try {
        const message = `Your Monay verification code is: ${otp}. This code will expire in 5 minutes.`;
        
        // Call Nudge SMS API
        const response = await axios.post(
          `${this.nudgeApiUrl}/v1/Nudge/SendSMS`,
          {
            to: phoneNumber,
            message: message,
            from: 'Monay'
  ...
  async sendEmailOTP(email, userId, userName = 'User') {
    try {
      ...
      try {
        // Use Nudge Send API with template
        const response = await axios.post(
          `${this.nudgeApiUrl}/v1/Nudge/Send`,
          {
            nudgeId: "5281", // Fixed Nudge ID for OTP emails
            channel: 0, // 0 for email
            toEmailAddress: email,
            recipientId: userName,
            toName: userName,
            mergeTags: [
              { tag: "OTP_CODE", value: otp },
