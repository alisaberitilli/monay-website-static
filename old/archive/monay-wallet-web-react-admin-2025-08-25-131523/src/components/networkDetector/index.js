...
export default class NetworkDetector extends PureComponent {
  constructor (props) {
    super(props)
  ...
  updateConnectionStatus = (isDisconnected = true) => {
    this.setState({ isDisconnected }, async () => {
      if (!this.state.imageIsCached) {
        try {
          const srcURL = `${this.state.baseUrl}/assets/images/no_internet_connection.svg` // by setting an src, you trigger browser download
          const base64Img = await getBase64OfsvgURL(srcURL)
          this.setState({
            imageIsCached: true,
            base64Img
          })
  ...
  render () {
    ...
    return (
      <Modal
        ...
        id='networkDetectorModal'
        className='networkDetectorModal'
        centered
      >
        <Modal.Body
          className='comment-list pt-0 mb-0 text-center'
        >
          <>
            {
