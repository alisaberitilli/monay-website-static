...
class FailedTransaction extends PureComponent {
  ...
  render() {
    ...
    return (
      <>
        <MetaTags
          title={`${config.NAME_TITLE} | ${t('transaction.failedTransaction')}`}
          description={`${t('transaction.failedTransaction')} of ${config.NAME_TITLE}`}
        />
              ...
              <div className='left-side'>
                <BreadCrumb
                  bredcrumbs={[
                    ...
                    {
                      name: t('transaction.failedTransaction')
                    }
                  ]}
                />
                <h2 className='page-title text-capitalize mb-0'>
                  {t('transaction.failedTransaction')}
                </h2>
              </div>
            ...
            </>
            {
              checkUserPermission(this.props.userData, 'transaction') ?
                <RemoteDataTable
                  ...
                  fileName={`Monay_Failed_Transactions_Listing_${currentDate()}.csv`}
                /> : <RemoteDataTable
                  data={data}
                  page={page}
                  sizePerPage={sizePerPage}
...
export default withTranslation()(connect(mapStateToProps)(FailedTransaction))
