...
// Monay Fiat (GPS) API Configuration
const MONAY_FIAT_API_URL = process.env.MONAY_FIAT_API_URL || 'https://pregps.monay.com/api';
const MONAY_FIAT_API_KEY = process.env.MONAY_FIAT_CARD_API_KEY || 'cec7602a-1a16-4f7e-86b2-1856536006bf';
const MONAY_FIAT_MERCHANT_ID = process.env.MONAY_FIAT_CARD_MERCHANT_ID || 'lpEGBQCW1mtX';
const MONAY_FIAT_ACH_MERCHANT_ID = process.env.MONAY_FIAT_ACH_MERCHANT_ID || '8DutmzBEHr4W';
...
interface MonayFiatTransaction {
...
export async function POST(request: NextRequest) {
  try {
    ...
    /* PRODUCTION CODE - Uncomment when Monay Fiat integration is ready

    if (!MONAY_FIAT_API_KEY) {
      console.error('Monay Fiat API key not configured');
      return NextResponse.json(
    ...
    // Create transaction with Monay Fiat (GPS)
    const monayFiatRequest = {
      merchantId: body.paymentMethod === 'bank' ? MONAY_FIAT_ACH_MERCHANT_ID : MONAY_FIAT_MERCHANT_ID,
      amount: body.amount,
      currency: 'USD',
