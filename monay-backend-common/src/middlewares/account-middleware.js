...
export default {
  ...
  async checkEmailExists(req, res, next) {
    try {
      ...
      // Skip email validation if not provided (optional for web, iOS, Android)
      if (!email) {
        next();
        return;
      }
  ...
  async checkPhoneNumberExists(req, res, next) {
    try {
      ...
      if (isEmail) {
        ...
        // Use mobile field for phone number
        where.mobile = username.startsWith('+') ? username : '+1' + username;
      }
  ...
  async checkUniquePhoneNumberSubadmin(req, res, next) {
    try {
      ...
      if (phoneNumber) {
        const mobile = phoneNumber.startsWith('+') ? phoneNumber : '+1' + phoneNumber;
        const user = await userRepository.findOne({ mobile });
