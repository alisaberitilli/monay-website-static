  ...
  */
  async userProfile(req, res, next) {
    try {
      ...
      if (user) {
        // Check if users are in same country based on mobile prefix
        const reqUserCountry = req.user.mobile?.substring(0, 3);
        const userCountry = user.mobile?.substring(0, 3);
        if (reqUserCountry === userCountry) {
...
*/
  ...
  /**
   * Get all consumer users for contact selection
   * @param {Object} req
   * @param {Object} res
   * @param {Function} next
   */
  async getAllConsumerUsers(req, res, next) {
    try {
      const { role } = req.query;
      
      // Create a request object to mimic admin user list request
      const mockReq = {
        query: {
          role: role || 'verified_consumer',
          limit: 100, // Get up to 100 users
          offset: 0
        },
        user: { id: 'system' } // Mock system user to avoid filtering
      };
      
      const result = await userRepository.getUserList(mockReq);
